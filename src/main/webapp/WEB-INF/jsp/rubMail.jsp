				...
				<div class="action">
					...
					<div class="pages">
						<table width="90%" border="0" cellspacing="0" cellpadding="0">
							...
							<td>${mail.sendtime}</td>
							<td><a onclick="deleteMail(${mail.id})">删除</a>|<a onclick="addMail(${mail.id})">还原</a></td>
							</tr>
							</c:forEach>
...
<script type="text/javascript">
			...
			/* function deleteMail(id) {
				var conf = confirm("确定执行此操作？");
				if(conf){
					location.href="${pageContext.request.contextPath}/user/deleteMail/"+id;
				}
			} */
			...
			function deleteMail(id){
				if(window.confirm("确定执行此操作？")){
					$.ajax({
						dataType : "text",
						type : "post",
						url :"${pageContext.request.contextPath}/user/deleteMail/"+id,
						success : function(){
							location.reload();
						}
					});
			...
			function addMail(id){
				if(window.confirm("确定执行此操作？")){
					$.ajax({
						dataType : "text",
						type : "post",
						url :"${pageContext.request.contextPath}/user/updateRubMailIsshow/"+id,
						success : function(){
							location.reload();
						}
					});
